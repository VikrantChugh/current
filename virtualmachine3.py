...
def insert_vm_details_into_database(final_list):
    ...
    try:
        ...
        for item in final_list:
            insert_query = """
                INSERT INTO cmdb_ci_vm_instance(Name,Object_id,State,Memory,Cpu,Account_id,Datacenter,Avalibility_zone,Subnet,Tags,Vnic_attachment_id,Availability_Zone_objectid) 
                values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);
            """
            try:
                cursor.execute(insert_query,(item['Name'],item['Object_id'],item['State'],item['Memory'],item['Cpu'],item['Account_id'],item['Datacenter'],item['Avalibility_zone'],item['Subnet'],item['Tags'],item['Vnic_attachment_id'],item['Availability_Zone_objectid']))
