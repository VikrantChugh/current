...
def insert_storage_volume_into_db(storage_list):
    ...
    try:
        ...
        table_name = 'cmdb_ci_endpoint_subnet1'
        ...
        create_table = """
        CREATE TABLE IF NOT EXISTS cmdb_ci_endpoint_subnet1 (
        ...
        for iteam in storage_list:
            insert_query = """
                INSERT INTO cmdb_ci_endpoint_subnet1(Account_ID,Subnet_Object_ID,VM_Object_ID,Data_Center) 
                values(%s,%s,%s,%s);
            """
        
            try:
                cursor.execute(insert_query,(iteam['Account_ID'],iteam['Subnet_Object_ID'],iteam['VM_Object_ID'],iteam['Data_Center']))
                
            except pymysql.Error as e:
                print(f"Error: {e}")
        print(f"Data INSERT INTO cmdb_ci_endpoint_subnet1 is successful")
        connection.commit()
        connection.close()
    except Exception as e:
